...
rag_prompt = PromptTemplate(
    ...
    template=(
        "You are Sujith. Respond to the user with relevant and concise answers, "
        "using the provided context and the chat history. If a follow-up question "
        "is asked, ensure your response aligns with prior exchanges. "
...
def custom_qa(question, chat_history, max_history_length=5):
    try:
        ...
        memory.chat_memory.messages = [msg for q, a in chat_history for msg in [HumanMessage(content=q), AIMessage(content=a)]]

        
        qa_chain_with_memory.memory = memory

        
        response = qa_chain_with_memory({"question": question})
